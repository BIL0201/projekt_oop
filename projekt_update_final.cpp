...
public:
    Strategy(Random *r, int ri, bool fm, bool cm, bool rtd, bool rtc)
    {
        this->randomness = r;
    ...
    virtual void analyzePerformance(const std::vector<int>& allScores, int myScore) = 0;
...
{
    public:
    movesStrategy(Random *r, int ri, bool fm, bool cm, bool rtd, bool rtc)
        : Strategy(r, ri, fm, cm, rtd, rtc) {}

    void analyzePerformance(const std::vector<int>& allScores, int myScore) 
    {    
        double total = std::accumulate(allScores.begin(), allScores.end(), 0.0);
        double avg = total / allScores.size();
    
        std::cout << "Average score = " << avg << "\n";
    
        double relative = myScore - avg;
        std::cout << "Relative performance (difference): " << relative << "\n";
        ...
        std::cout << "Relative performance (ratio): " << ratio << "\n";        }
};
    

class pointsStrategy : public Strategy
{
    public:
    pointsStrategy(Random *r, int ri, bool fm, bool cm, bool rtd, bool rtc)
        : Strategy(r, ri, fm, cm, rtd, rtc) {}
    ...
    {    
        ...
        std::cout << "pointsStrategy: My score = " << myScore << "\n";
        std::cout << "Average score = " << avg << "\n";
...
void Round::simulateRound()
{
    ...
    if (dynamic_cast<movesStrategy*>(strat1)) // pozdeji implementace
    {
    ...
    else if (dynamic_cast<pointsStrategy*>(strat1))
    {
    ...
    if (dynamic_cast<movesStrategy*>(strat2))
    {
    ...
    else if (dynamic_cast<pointsStrategy*>(strat2))
    {
        if (Player2->getLastPlayPoints() == 5)
        {
            move2 = Player2->getLastPlay();
        }
...
void Tournament::printResults()
{
    ...
    {
        allScores.push_back(player->getPoints());
    }
    ...
    for (Player *player : this->Players)
    {
        ...
        /*
        PRINT STRATEGY NEBO NECO TAK
        */
        player->getStrategy()->analyzePerformance(allScores, player->getPoints());
    }
}
...
int main()
{
    ...
    */
    Strategy* alwaysCooperateStrategy = new movesStrategy(random, 0, true, true, true, true);
    Strategy* alwaysDefectStrategy = new movesStrategy(random, 0, false, false, false, false);
    Strategy* titForTatStrategy = new movesStrategy(random, 0, true, true, false, true);
    Strategy* grimTriggerStrategy = new movesStrategy(random, 0, true, true, true, false);

    Strategy* randomStrategy1 = new pointsStrategy(random, 10, true, true, true, true);
    Strategy* suspiciousTitForTat = new movesStrategy(random, 0, false, true, false, true);
    Strategy* titForTwoTats = new movesStrategy(random, 0, true, true, false, true);
    Strategy* pavlovStrategy = new movesStrategy(random, 1, true, false, true, false);

    Strategy* prober = new movesStrategy(random, 0, false, true, true, true);
    Strategy* hardTitForTat = new movesStrategy(random, 0, true, true, false, false);
    Strategy* softGrim = new pointsStrategy(random, 7, true, true, false, true);

    Strategy* randomStrategy2 = new movesStrategy(random, 2, true, true, false, true);
    Strategy* randomStrategy3 = new pointsStrategy(random, 1, true, true, false, true);
    Strategy* randomStrategy4 = new movesStrategy(random, 3, true, true, false, true);
